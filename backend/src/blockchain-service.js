...
class BlockchainService {
  constructor() {
    ...
    this.RPC_URL = process.env.BLOCKCHAIN_RPC_URL; // Fixed: was BASE_SEPOLIA_RPC
    ...
    console.log(
      "Backend Signer:",
      this.signer ? this.signer.address : "Not configured",
    );
  }
  ...
  async emergencyCancelBooking(bookingId, reason) {
    try {
      ...
      const bookingIdBytes = ethers.keccak256(ethers.toUtf8Bytes(bookingId));
      const tx = await this.contractWithSigner.emergencyCancelBooking(
        bookingIdBytes,
        reason,
      );
