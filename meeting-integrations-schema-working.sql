-- Meeting Integrations Database Schema (Working - With Type Casting)
...
-- 6. Create RLS policies with proper type casting
DO $$
DECLARE
    user_id_type text;
BEGIN
    ...
    -- Create policies based on the user_id type
    IF user_id_type = 'uuid' THEN
        -- UUID policies - cast text to UUID
        ...
        RAISE NOTICE 'Created RLS policies for UUID user_id type';
    ELSE
        -- TEXT policies - no casting needed
        CREATE POLICY "Users can view own meeting integrations" ON user_meeting_integrations
        ...
        RAISE NOTICE 'Created RLS policies for TEXT user_id type';
    END IF;
END $$;
...
BEGIN
    ...
    SELECT EXISTS (
        ...
        WHERE table_name = 'user_meeting_integrations'
    ) INTO integrations_exists;
    ...
    RAISE NOTICE 'user_meeting_integrations table exists: %', integrations_exists;
    RAISE NOTICE '==========================';
END $$;